# utils_openclaw.py
# Shared utilities for OpenClaw/Moltbook analysis
#
...
#     comments.csv
#
# NOTE (Step 1 terminology alignment):
# - We use Directive Intensity (DI) as the neutral, lexicon-based proxy for directive/action-inducing language intensity.
# - For backward compatibility with older scripts, we keep 'airs' as an alias of 'di' (same numeric values).
...
class Paths:
    ...
    # derived
    codes_dir: Path = Path(__file__).resolve().parent          # .../Codes
    root_dir: Path = Path(__file__).resolve().parent.parent    # project root
...
# ----------------------------
# DI (Directive Intensity)
# ----------------------------
# Two pattern categories:
# - ACTION_PATTERNS: action-oriented / instructional phrasing
# - SENSITIVE_PATTERNS: sensitive/execution-related terms
#
# We score DI as the capped count of matched patterns across both categories (cap=10).
ACTION_PATTERNS = [
    r"\bdo this\b",
...
# Backward-compatible name (old manuscripts/scripts)
...
# ----------------------------
# Comment response type classifier
# ----------------------------
# Canonical (machine) labels for the manuscript:
#   affirmation / corrective / adversarial / neutral
#
# For safety, we also store legacy labels:
#   endorse / enforce / toxic / other
...
def add_response_types(comments: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep legacy mapping for older scripts / sanity checks
...
# ----------------------------
# DI bucketing helper (used elsewhere)
# ----------------------------
...
if __name__ == "__main__":
    p = Paths()
    a, po, c = load_all(p)
    print("[OK] agents_csv:", p.agents_csv)
    print("[OK] posts_csv:", p.posts_csv)
    print("[OK] comments_csv:", p.comments_csv)
    print("[OK] results_dir:", p.results_dir)
    print("[OK] figures_dir:", p.figures_dir)
    print("Agents:", a.shape, "Posts:", po.shape, "Comments:", c.shape)
    print("DI preview:", po["di"].describe())
    print("Response types:", c["response_type"].value_counts().to_dict())
