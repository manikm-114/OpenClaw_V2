# di_robustness_variant.py
# ------------------------------------------------------------
# Step 4B: Lightweight robustness
# Variant DI lexicon: remove broad technical tokens; keep directive phrasing core.
# Writes:
#   results/di_validation_summary.txt
#   results/di_robustness_bins_main.csv
#   results/di_robustness_bins_variant.csv
# ------------------------------------------------------------
...
# Variant: remove broad tech tokens from action patterns
DROP_SUBSTRINGS = [
    r"\bpython\b",
    r"\bbash\b",
    r"\bpowershell\b",
...
def make_variant_action_patterns():
    ...
    for p in ACTION_PATTERNS:
        # drop if it exactly matches any of the broad tech patterns above
...
def summarize_bins(posts: pd.DataFrame, comments: pd.DataFrame, di_col: str, n_pos_bins=3) -> pd.DataFrame:
    # comments: is_corrective already present in earlier pipeline via response_type
    # merge di onto comments
...
def main():
    ...
    # Simple validation summary text
    txt = []
