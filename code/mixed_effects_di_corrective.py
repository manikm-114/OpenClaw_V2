# mixed_effects_di_corrective.py
# ------------------------------------------------------------
# Step 3: Mixed-effects logistic regression (post-level random intercept)
#
# Primary target:
#   GLMM: logit P(is_corrective_ij = 1) = beta0 + beta1 * DI_j + u_j
#         u_j ~ Normal(0, sigma_post^2)
#
# Data:
#   - posts.csv provides DI at post level (computed in utils_openclaw.load_posts)
#   - comments.csv provides response_type at comment level (computed in utils_openclaw.load_comments)
#   - merge DI onto comments by post_id
#
# Outputs (in ../results/):
#   - mixed_effects_di_corrective.txt
#   - mixed_effects_di_corrective_fixed_effects.csv
#   - mixed_effects_di_corrective_random_effects_summary.csv
#   - mixed_effects_di_corrective_SI_paragraph.txt
#
# Notes:
#   - In Python, frequentist logistic GLMM support is limited in statsmodels.
#   - We attempt BinomialBayesMixedGLM (variational Bayes) as the primary GLMM.
#   - If unavailable, we run a clustered GEE logistic as a robustness fallback.
# ------------------------------------------------------------
...
# Configuration
# ----------------------------
USE_STANDARDIZED_DI = True   # standardize DI to mean 0, sd 1 for stable estimation
MIN_COMMENTS_PER_POST = 1    # keep all posts with >=1 comment
INCLUDE_LOG_DI = False       # optional alternative: use log1p(DI); keep False unless you decide
RANDOM_SEED = 7
...
def build_comment_level_df(paths: Paths) -> pd.DataFrame:
    ...
    # filter posts by comment count if desired
...
# ----------------------------
# GLMM via BinomialBayesMixedGLM (if available)
# ----------------------------
def fit_glmm_bayes_mixed(df: pd.DataFrame):
    ...
    # --- Fixed effects: use fe_mean/fe_sd if present (common in statsmodels) ---
    if hasattr(res, "fe_mean") and hasattr(res, "fe_sd"):
        fe_names = model.exog_names  # usually ["Intercept", "x"]
        fe_params = np.asarray(res.fe_mean, dtype=float)
        fe_sds = np.asarray(res.fe_sd, dtype=float)
    else:
        # fallback: try params + covariance if available
    ...
    # Build fixed effects table (approx)
    fe_rows = []
    ...
    # Many versions expose vcp_mean (variance component mean). Keep it approximate.
    ...
    # SI paragraph from fixed effect of x
...
# ----------------------------
# Fallback: Clustered GEE logistic
# ----------------------------
